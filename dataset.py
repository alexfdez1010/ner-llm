...
class Instance:
    ...
    def get_bio_annotations(self) -> list[str]:
        ...
        for entity in self.entities:
            ...
            if start is not None and end is not None:  # Only process if valid token indexes found
                annotations[start] = f"B-{entity.category}"
                for i in range(start + 1, end + 1):
                    annotations[i] = f"I-{entity.category}"
        return annotations

    def _get_token_indexes_from_span(self, span: tuple[int, int]) -> tuple[int, int] | tuple[None, None]:
        """
        ...
        tokens: list[tuple[int, int, int, int]] = []  # (ws_start, token_start, token_end, ws_end)
        current_pos = 0
