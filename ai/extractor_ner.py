...
"""
        ...
        self.prompt = PromptTemplate(
            template=self.template, input_variables=["categories", "examples_text"]
        )

    def extract_entities(
        self, categories: list[Category], text: str, examples: list[str] | None = None
    ) -> list[Entity]:
        ...
        system_prompt = self.prompt.format(
            categories=categories_text, examples_text=examples_text
        )
        ...
        for line in raw_output.strip().split("\n"):
            ...
            while True:
                ...
                entities.append(
                    Entity(
                        category=category, entity=entity, span=(pos, pos + len(entity))
                    )
                )
                pos += 1
